...
export function generateProgressReport(data: ReportData): jsPDF {
  try {
    ...
    if (data.user.nhsNumber) {
      doc.text(`Waitlist Companion Number: ${data.user.nhsNumber}`, 20, yPos);
      yPos += 6;
    }
...
export function generateNhsPrepReport(data: NhsPrepData): jsPDF {
  try {
    ...
    doc.setFontSize(20);
    doc.setFont('helvetica', 'bold');
    doc.text('Waitlist Companion Mental Health Services Preparation Guide', 20, yPos);
    yPos += 10;
    ...
    yPos += 6;
    doc.text(`Waitlist Companion Readiness Score: ${data.overallReadiness}%`, 20, yPos);
    yPos += 6;
    ...
    // Specialist Care Readiness Scores
    checkPageBreak(30);
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Waitlist Companion Transition Readiness Assessment', 20, yPos);
    yPos += 10;
    ...
    if (data.advocacyPrep.concerns) {
      doc.setFont('helvetica', 'bold');
      doc.text('Concerns About Waitlist Companion Services:', 20, yPos);
      yPos += 6;
      doc.setFont('helvetica', 'normal');
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text('Generated by Waitlist Companion™ Preparation Guide', 20, 285);
    doc.text(`Page ${doc.internal.getNumberOfPages()}`, 160, 285);

    return doc;

  } catch (error) {
    throw new Error(`Waitlist Companion Prep PDF generation failed: ${error.message}`);
  }
}
...
const formatValue = (value: any): string => {
  ...
  // Handle checkbox objects with labels
  if (typeof value === "object" && value !== null && 'label' in value && 'checked' in value) {
    return value.checked ? `✓ ${value.label}` : value.label;
  }
...
const renderKeyValue = (writer: PdfWriter, data: any, indent = 0) => {
  ...
  // Check if this looks like worksheet checkbox data (keys like "Physical-0", "Cognitive-1", etc.)
  const isCheckboxData = entries.every(([key, value]) => {
    return (typeof value === 'boolean' || (typeof value === 'object' && value !== null && 'label' in value && 'checked' in value)) &&
      key.includes('-');
  });

  if (isCheckboxData) {
    // Group by category and show only checked items
    const grouped: Record<string, string[]> = {};

    entries.forEach(([key, value]) => {
      const isChecked = typeof value === 'boolean' ? value : value?.checked;
      if (!isChecked) return; // Skip unchecked items

      const dashIndex = key.lastIndexOf('-');
      const category = dashIndex > 0 ? key.substring(0, dashIndex) : key;
    ...
    if (Object.keys(grouped).length === 0) {
      writer.addText("None selected", { indent });
      return;
    }

    Object.entries(grouped).forEach(([category, items]) => {
      writer.addText(`${formatKey(category)}:`, { indent, bold: true });
      items.forEach(item => {
        writer.addText(`✓ ${item}`, { indent: indent + 1 });
      });
    });
    return;
  }
...
const renderReportHeader = (writer: PdfWriter, user: User, title: string) => {
  ...
  if (user.nhsNumber) {
    writer.addText(`Waitlist Companion Number: ${user.nhsNumber}`, { indent: 1 });
  }
  writer.addGap();
};
